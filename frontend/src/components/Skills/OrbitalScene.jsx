/**
 * OrbitalScene.jsx — Système orbital 3D pour les compétences
 *
 * - Canvas React Three Fiber
 * - Anneaux orbitaux par catégorie (torusGeometry)
 * - Planètes-compétences (sphereGeometry, taille ∝ niveau)
 * - Parallaxe souris sur la scène globale
 * - Zoom molette
 * - Hover planète → tooltip détaillé
 * - Clic catégorie / bouton filtre → isolation de l'anneau
 */
...
const RADII  = [2.4, 3.9, 5.4, 6.9, 8.4];
const SPEEDS = [0.0042, -0.003, 0.0026, -0.002, 0.0016];
const TILTS  = [
  [ 0.28, 0,  0.10],
  [-0.38, 0, -0.18],
  [ 0.45, 0,  0.28],
...
const hex = (theme) => THEME_HEX[theme] ?? '#FF1744';

/* ============================================
   Central Sphere
   ============================================ */
function CentralSphere({ theme }) {
  const ref  = useRef();
...
function GlowRing({ theme, radius }) {
  const ref = useRef();
  const col = useMemo(() => new THREE.Color(hex(theme)), [theme]);
...
/* ============================================
   Skill Planet
   ============================================ */
function SkillPlanet({ skill, angle, radius, theme, onHover, onLeave, isHovered }) {
  const ref = useRef();
  const col = useMemo(() => new THREE.Color(hex(theme)), [theme]);
...
function OrbitalRing({
  ...
  return (
    <group rotation={tilt} visible={isVisible}>
      ...
      {/* Category label — à droite de l'anneau */}
      <Html
      ...
      {/* Groupe rotatif pour les planètes */}
      <group ref={rotRef}>
        {skills.map((skill, i) => (
          <SkillPlanet
...
/* ============================================
   Scene (inside Canvas)
   ============================================ */
function Scene({ categories, theme, selectedCategory, setSelectedCategory, hoveredSkill, setHoveredSkill }) {
